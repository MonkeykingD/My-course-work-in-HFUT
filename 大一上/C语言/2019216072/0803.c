...
void ReverseLiList(struct student *);
 ...
 *  return      :No. 
 --------------------------------------------------------*/
void ReverseLiList(struct student *sp)
{
	struct student *spCur, *spPrv = NULL;
	spCur = sp -> pNext;
	while(spCur!= NULL)
	{
        struct student *next = spCur -> pNext;
        spCur -> pNext = spPrv;
        spPrv = spCur;
		spCur = next;
	}
